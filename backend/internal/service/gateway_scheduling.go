...
// Anthropic 与 OpenAI 一并放行。Gemini 与 Antigravity 的生成入口走同一套通用调度
// （见 handler.GeminiV1BetaModels → SelectAccountWithLoadAwareness），同样按此放行。
//
// 注意本函数只覆盖通用网关。openai / grok / kimi / zhipu / deepseek 分组的
// Responses、Chat 与 Messages 由 OpenAIGatewayService 独立调度，其路由接线见
// openai_model_routing.go。
//
