...
func TestApplyMigrationsFS_NonTransactionalMigration_OpsSystemLogsHostIndexDropsInvalidIndexBeforeRetry(t *testing.T) {
	...
	fsys := fstest.MapFS{
		opsSystemLogsHostIndexMigration: &fstest.MapFile{Data: []byte(`
...
func TestApplyMigrationsFS_NonTransactionalMigration_UsersEmailAliasDedupIndexDropsInvalidIndexBeforeRetry(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	defer func() { _ = db.Close() }()
	...
	fsys := fstest.MapFS{
		usersEmailAliasDedupIndexMigration: &fstest.MapFile{Data: []byte(`
